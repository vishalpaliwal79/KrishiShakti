...
def detect_topic(message_lower, original_message):
    ...
    if any(word in message_lower for word in ['health', 'crop', 'plant', 'growing', 'condition']):
        return 'health'
    # Hindi: स्वास्थ्य, फसल, पौधा
    if any(word in original_message for word in ['स्वास्थ्य', 'फसल', 'पौधा', 'सेहत']):
        return 'health'
    # Punjabi: ਸਿਹਤ, ਫਸਲ, ਪੌਦਾ
    if any(word in original_message for word in ['ਸਿਹਤ', 'ਫਸਲ', 'ਪੌਦਾ']):
        return 'health'
    
    # Water - English
    if any(word in message_lower for word in ['water', 'irrigat', 'wet', 'dry', 'moisture', 'pani']):
        return 'water'
    # Hindi: पानी, सिंचाई, नमी
    if any(word in original_message for word in ['पानी', 'सिंचाई', 'नमी', 'गीला', 'सूखा']):
        return 'water'
    # Punjabi: ਪਾਣੀ, ਸਿੰਚਾਈ, ਨਮੀ
    if any(word in original_message for word in ['ਪਾਣੀ', 'ਸਿੰਚਾਈ', 'ਨਮੀ', 'ਗਿੱਲਾ', 'ਸੁੱਕਾ']):
        return 'water'
    ...
    if any(word in message_lower for word in ['fertiliz', 'nutrient', 'npk', 'feed', 'manure', 'khad']):
        return 'fertilizer'
    # Hindi: खाद, उर्वरक
    if any(word in original_message for word in ['खाद', 'उर्वरक', 'पोषक']):
        return 'fertilizer'
    # Punjabi: ਖਾਦ, ਉਰਵਰਕ
    if any(word in original_message for word in ['ਖਾਦ', 'ਉਰਵਰਕ', 'ਪੋਸ਼ਕ']):
        return 'fertilizer'
    ...
    if any(word in message_lower for word in ['pest', 'insect', 'bug', 'aphid', 'keeda', 'keet']):
        return 'pest'
    # Hindi: कीट, कीड़ा
    if any(word in original_message for word in ['कीट', 'कीड़ा', 'कीड़े']):
        return 'pest'
    # Punjabi: ਕੀੜਾ, ਕੀਟ
    if any(word in original_message for word in ['ਕੀੜਾ', 'ਕੀਟ', 'ਕੀੜੇ']):
        return 'pest'
    ...
    if any(word in message_lower for word in ['disease', 'sick', 'yellow', 'spot', 'fungus', 'bimari', 'rog']):
        return 'disease'
    # Hindi: बीमारी, रोग
    if any(word in original_message for word in ['बीमारी', 'रोग', 'पीला', 'धब्बा']):
        return 'disease'
    # Punjabi: ਬਿਮਾਰੀ, ਰੋਗ
    if any(word in original_message for word in ['ਬਿਮਾਰੀ', 'ਰੋਗ', 'ਪੀਲਾ', 'ਧੱਬਾ']):
        return 'disease'
    
    # Temperature - English
    if any(word in message_lower for word in ['hot', 'cold', 'temperature', 'heat', 'cool', 'garmi', 'thand']):
        return 'temperature'
    # Hindi: गर्मी, ठंड, तापमान
    if any(word in original_message for word in ['गर्मी', 'ठंड', 'तापमान']):
        return 'temperature'
    # Punjabi: ਗਰਮੀ, ਠੰਡ, ਤਾਪਮਾਨ
    ...
    if any(word in message_lower for word in ['soil', 'dirt', 'ground', 'earth', 'mitti', 'zameen']):
        return 'soil'
    # Hindi: मिट्टी, जमीन, भूमि
    if any(word in original_message for word in ['मिट्टी', 'जमीन', 'भूमि']):
        return 'soil'
    # Punjabi: ਮਿੱਟੀ, ਜ਼ਮੀਨ, ਧਰਤੀ
    if any(word in original_message for word in ['ਮਿੱਟੀ', 'ਜ਼ਮੀਨ', 'ਧਰਤੀ']):
        return 'soil'
    
    # Weather - English
    if any(word in message_lower for word in ['weather', 'rain', 'sun', 'wind', 'mausam', 'barish']):
        return 'weather'
    # Hindi: मौसम, बारिश, धूप
    if any(word in original_message for word in ['मौसम', 'बारिश', 'धूप', 'हवा']):
        return 'weather'
    # Punjabi: ਮੌਸਮ, ਮੀਂਹ, ਧੁੱਪ
    if any(word in original_message for word in ['ਮੌਸਮ', 'ਮੀਂਹ', 'ਧੁੱਪ', 'ਹਵਾ']):
        return 'weather'
    
    # Planting - English
    if any(word in message_lower for word in ['plant', 'seed', 'sow', 'grow', 'beej', 'bona']):
        return 'planting'
    # Hindi: बीज, बोना, लगाना
    if any(word in original_message for word in ['बीज', 'बोना', 'लगाना', 'उगाना']):
        return 'planting'
    # Punjabi: ਬੀਜ, ਬੀਜਣਾ, ਲਾਉਣਾ
    if any(word in original_message for word in ['ਬੀਜ', 'ਬੀਜਣਾ', 'ਲਾਉਣਾ', 'ਉਗਾਉਣਾ']):
        return 'planting'
    
    # Harvest - English
    if any(word in message_lower for word in ['harvest', 'pick', 'collect', 'ready', 'ripe', 'kaatna', 'fasal']):
        return 'harvest'
    # Hindi: कटाई, काटना, तोड़ना
    if any(word in original_message for word in ['कटाई', 'काटना', 'तोड़ना', 'पकना']):
        return 'harvest'
    # Punjabi: ਵਾਢੀ, ਕੱਟਣਾ, ਤੋੜਨਾ
    if any(word in original_message for word in ['ਵਾਢੀ', 'ਕੱਟਣਾ', 'ਤੋੜਨਾ', 'ਪੱਕਣਾ']):
        return 'harvest'
...
def get_translations(language):
    ...
    translations = {
        'english': {
            ...
            'perfect': 'Perfect',
            'pure': 'Pure',
            'health_title': '🌱 **Crop Health Score:',
            'excellent_condition': '✅ Your crops are in excellent condition!',
            'need_attention': '⚠️ Your crops are doing okay but need attention.',
            'immediate_care': '🚨 Your crops need immediate care!',
            'current_status': '**Current Status:**',
            'water_title': '💧 **Irrigation Advice:**',
            'urgent': '🚨 **URGENT:**',
            'water_now': 'Water NOW!',
            'fertilizer_title': '🌿 **Fertilizer Recommendation:**',
            'pest_title': '🐛 **Pest Control Guide:**',
            'disease_title': '🔬 **Plant Disease Guide:**',
            'temp_title': '🌡️ **Temperature Management:**',
            'soil_title': '🌍 **Soil Management:**',
            'weather_title': '🌤️ **Weather-Based Farming:**',
            'planting_title': '🌱 **Planting Guide:**',
            'harvest_title': '🌾 **Harvesting Guide:**',
            'general_title': '🌾 **Farming Assistant:**',
        },
        'hindi': {
            ...
            'low': 'कम',
            'high': 'ज्यादा',
            'too_high': 'बहुत ज्यादा',
            'too_low': 'बहुत कम',
            'perfect': 'परफेक्ट',
            'pure': 'शुद्ध',
            'health_title': '🌱 **फसल स्वास्थ्य स्कोर:',
            'excellent_condition': '✅ आपकी फसलें बहुत अच्छी स्थिति में हैं!',
            'need_attention': '⚠️ आपकी फसलें ठीक हैं लेकिन ध्यान की जरूरत है।',
            'immediate_care': '🚨 आपकी फसलों को तुरंत देखभाल की जरूरत है!',
            'current_status': '**वर्तमान स्थिति:**',
            'water_title': '💧 **सिंचाई सलाह:**',
            'urgent': '🚨 **जरूरी:**',
            'water_now': 'अभी पानी दें!',
            'fertilizer_title': '🌿 **खाद की सिफारिश:**',
            'pest_title': '🐛 **कीट नियंत्रण गाइड:**',
            'disease_title': '🔬 **पौधों की बीमारी गाइड:**',
            'temp_title': '🌡️ **तापमान प्रबंधन:**',
            'soil_title': '🌍 **मिट्टी प्रबंधन:**',
            'weather_title': '🌤️ **मौसम आधारित खेती:**',
            'planting_title': '🌱 **बुवाई गाइड:**',
            'harvest_title': '🌾 **कटाई गाइड:**',
            'general_title': '🌾 **खेती सहायक:**',
        },
        'punjabi': {
            'sensor_title': '📊 **ਮੌਜੂਦਾ ਸੈਂਸਰ ਰੀਡਿੰਗ:**',
            'temperature': 'ਤਾਪਮਾਨ',
            'humidity': 'ਨਮੀ',
            'air_quality': 'ਹਵਾ ਦੀ ਗੁਣਵੱਤਾ',
            'soil_moisture': 'ਮਿੱਟੀ ਦੀ ਨਮੀ',
            'water_quality': 'ਪਾਣੀ ਦੀ ਗੁਣਵੱਤਾ',
            'optimal': 'ਸਭ ਤੋਂ ਵਧੀਆ',
            'good': 'ਚੰਗਾ',
            'excellent': 'ਬਹੁਤ ਵਧੀਆ',
            'poor': 'ਮਾੜਾ',
            'low': 'ਘੱਟ',
            'high': 'ਜ਼ਿਆਦਾ',
            'too_high': 'ਬਹੁਤ ਜ਼ਿਆਦਾ',
            'too_low': 'ਬਹੁਤ ਘੱਟ',
            'perfect': 'ਪਰਫੈਕਟ',
            'pure': 'ਸ਼ੁੱਧ',
            'health_title': '🌱 **ਫਸਲ ਸਿਹਤ ਸਕੋਰ:',
            'excellent_condition': '✅ ਤੁਹਾਡੀਆਂ ਫਸਲਾਂ ਬਹੁਤ ਵਧੀਆ ਹਾਲਤ ਵਿੱਚ ਹਨ!',
            'need_attention': '⚠️ ਤੁਹਾਡੀਆਂ ਫਸਲਾਂ ਠੀਕ ਹਨ ਪਰ ਧਿਆਨ ਦੀ ਲੋੜ ਹੈ।',
            'immediate_care': '🚨 ਤੁਹਾਡੀਆਂ ਫਸਲਾਂ ਨੂੰ ਤੁਰੰਤ ਦੇਖਭਾਲ ਦੀ ਲੋੜ ਹੈ!',
            'current_status': '**ਮੌਜੂਦਾ ਸਥਿਤੀ:**',
            'water_title': '💧 **ਸਿੰਚਾਈ ਸਲਾਹ:**',
            'urgent': '🚨 **ਜ਼ਰੂਰੀ:**',
            'water_now': 'ਹੁਣੇ ਪਾਣੀ ਦਿਓ!',
            'fertilizer_title': '🌿 **ਖਾਦ ਦੀ ਸਿਫਾਰਸ਼:**',
            'pest_title': '🐛 **ਕੀੜੇ ਕੰਟਰੋਲ ਗਾਈਡ:**',
            'disease_title': '🔬 **ਪੌਦਿਆਂ ਦੀ ਬਿਮਾਰੀ ਗਾਈਡ:**',
            'temp_title': '🌡️ **ਤਾਪਮਾਨ ਪ੍ਰਬੰਧਨ:**',
            'soil_title': '🌍 **ਮਿੱਟੀ ਪ੍ਰਬੰਧਨ:**',
            'weather_title': '🌤️ **ਮੌਸਮ ਅਧਾਰਤ ਖੇਤੀ:**',
            'planting_title': '🌱 **ਬੀਜਣ ਗਾਈਡ:**',
            'harvest_title': '🌾 **ਵਾਢੀ ਗਾਈਡ:**',
            'general_title': '🌾 **ਖੇਤੀ ਸਹਾਇਕ:**',
        }
    }
